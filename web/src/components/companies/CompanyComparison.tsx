...
export function CompanyComparison({ companies }: { companies: CompanySummary[] }) {
  ...
  const visible = companies.filter(c => (category === "All categories" || c.category === category) && (track === "Both workstreams" || c.track === track) && `${c.name} ${c.country} ${c.offer} ${c.category} ${c.yc ? "YC Y Combinator" : ""}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return <section aria-labelledby="company-table-title">
    ...
    <div className="co-table-wrap"><table className="co-table">
      ...
      <thead><tr><th scope="col">Company / market</th><th scope="col">What it does</th><th scope="col">What the evidence shows</th><th scope="col">What we can learn</th></tr></thead>
      <tbody>{visible.map(c => <Fragment key={c.id}>
        ...
        <tr id={`detail-${c.id}`} hidden={!expanded.includes(c.id)} className="co-detail"><td colSpan={4}><div className="co-detail-grid">
          <div><h3>Who pays?</h3><p>{c.payer}</p><h3>How the service works</h3><p>{c.workflow}</p><h3>The work behind the technology</h3><p>{c.burden}</p></div>
